    ...
    <div class="selection" ></div >
      ...
      <div class="news panel" >
        <div class="post_content" >
          <div class="post_info" >
            <a href="http://{{$blog->name}}.{{Config::get('database.domain')}}" > {{json_decode($blog->config)->title}} </a >
          </div >
...
@if($post->rebloggedPost->originalPost->regularPost['type'] == 'text')
    <div class="feed clearfix" >
      ...
      <div class="news panel" >
        <div class="post_content" >
          <div class="post_info" >
            <a href="http://{{$blog->name}}.{{Config::get('database.domain')}}" > {{json_decode($blog->config)->title}} </a > <i class="glyphicon glyphicon-retweet"></i> <a href="http://{{$post->rebloggedPost->originalPost->blog->name}}.{{Config::get('database.domain')}}"> {{json_decode($post->rebloggedPost->originalPost->blog->config)->title}} </a>
          </div >
...
<div class="blogger" >
        <a href="http://{{$blogger->name}}.{{Config::get('database.domain')}}" class="avatar">
          <img src="{{ asset(Image::path('/'.json_decode($blogger->config)->picture,'resizeCrop',64,64)) }}" class="dp">
        </a >
	 <a href="http://{{$blogger->name}}.{{Config::get('database.domain')}}" class="name">
		 {{$blogger->name}}
	</a>
</div>
